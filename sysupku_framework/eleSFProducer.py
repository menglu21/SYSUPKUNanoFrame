...
class eleSFProducer(Module):
  def __init__( self , year ):
    self.year = year
    self.reco_input1 = "ele_RECO_low.root"
    self.reco_input2 = "ele_RECO_high.root"
    self.SF_location_path = "%s/src/PhysicsTools/NanoAODTools/python/postprocessing/data_SF/year%s/" %(os.environ['CMSSW_BASE'], self.year)
    print 'SF location:', self.SF_location_path

  def beginJob(self):
    print 'begin to set Electron RECO and ID SF --->>>'
  ...
  def endJob(self):
    ...
    print 'finish setting Electron RECO and ID SF --->>>'
    
  def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
    self.out = wrappedOutputTree
    self.out.branch('Electron_RECO_SF','F', lenVar='nElectron')
    self.out.branch('Electron_RECO_SF_err','F', lenVar='nElectron')
#    self.out.branch('Electron_ID_SF','F')
#    self.out.branch('Electron_ID_SF_err','F')
  ...
  def analyze(self, event):
    ...
    if not (len(electrons)>0): return False
    Electron_RECO_SF = []
    Electron_RECO_SF_err = []
    ...
    for iele in range(0, len(electrons)):
      if electrons[iele].pt < 20: 
        ...
        Electron_RECO_SF_err.append(self.reco_th2f_low.GetBinError(self.reco_th2f_low.FindBin(electrons[iele].eta, electrons[iele].pt)))
      if electrons[iele].pt >= 20 and electrons[iele].pt < 500:
        ...
        Electron_RECO_SF_err.append(self.reco_th2f_high.GetBinError(self.reco_th2f_high.FindBin(electrons[iele].eta, electrons[iele].pt)))
      if electrons[iele].pt >= 500:
        ...
        Electron_RECO_SF_err.append(self.reco_th2f_high.GetBinError(self.reco_th2f_high.FindBin(electrons[iele].eta, 499)))
     
    self.out.fillBranch('Electron_RECO_SF', Electron_RECO_SF)
    self.out.fillBranch('Electron_RECO_SF_err', Electron_RECO_SF_err)
    return True

eleSF2016 = lambda: eleSFProducer("2016")
eleSF2017 = lambda: eleSFProducer("2017")
eleSF2018 = lambda: eleSFProducer("2018")
