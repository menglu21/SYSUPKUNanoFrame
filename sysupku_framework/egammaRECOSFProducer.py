...
class egammaRECOSFProducer(Module):
...
#    print 'SF location:', self.SF_location_path
  ...
  def analyze(self, event):
    ...
    if ((len(electrons)+len(photons))>0): pass
    Electron_RECO_SF = []
    Electron_RECO_SFerr = []
    Photon_RECO_SF = []
    Photon_RECO_SFerr = []
      ...
      if len(photons)>0:
        for ipho in range(0, len(photons)):
          ...
          if photons[ipho].pt >= 500:
            ...
            Photon_RECO_SFerr.append(self.reco_th2f_high.GetBinError(self.reco_th2f_high.FindBin(electrons[iele].eta, 499)))
...
egammaRECOSF2016 = lambda: egammaRECOSFProducer("2016")
egammaRECOSF2017 = lambda: egammaRECOSFProducer("2017")
egammaRECOSF2018 = lambda: egammaRECOSFProducer("2018")
